...
def generate_video(
    ...
    #pipe.scheduler.set_timesteps(num_inference_steps,"cuda")
    ...
    video = pipe(
        ...
        generator=torch.Generator().manual_seed(seed_1),  # Set the seed for reproducibility
    ).frames[0]

    os.makedirs(f"sample_video_ipoc5b/",exist_ok=True)
    export_to_video(video, f"sample_video_ipoc5b/{str(seed_1)}_{prompt[:50]}.mp4", fps=8)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model_path", type=str, default="Fudan-FUXI/IPOC-5B-v1.0", help="The path of the pre-trained model to be used"
    )
    parser.add_argument(
